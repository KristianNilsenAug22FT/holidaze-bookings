...
function VenueManagerDashboard({ isVenueManager, onRegisterAsVenueManager }) {
    ...
    const handleCreateVenue = async (e) => {
        ...
        try {
            ...
            setError('Failed to create venue. Please check the input data and try again.');
        }
    };
    ...
    const handleDeregister = async () => {
        const confirmDeregister = window.confirm("Are you sure you want to de-register as a venue manager? This will delete all your venues.");
        ...
        try {
            // Delete all venues
            await Promise.all(venues.map(venue => fetch(`https://v2.api.noroff.dev/holidaze/venues/${venue.id}`, {
                method: 'DELETE',
                headers
            })));
            ...
            {isVenueManager ? (
                <>
                            ...
                            {venues.map(venue => (
                                <li key={venue.id} className="venue-item">
                                    <div className="venue-details">
                                        <span>Venue: <Link to={`/venue/${venue.id}`}>{venue.name}</Link></span>
                                    ...
                                    <div className="venue-actions">
                                        ...
                                        <div className="dropdown">
                                            <button className="btn btn-secondary dropdown-toggle" type="button" id={`dropdownMenuButton-${venue.id}`} data-bs-toggle="dropdown" aria-expanded="false">
                                                Bookings
                                            </button>
                                            <ul className="dropdown-menu" aria-labelledby={`dropdownMenuButton-${venue.id}`}>
                                                {venue.bookings && venue.bookings.length > 0 ? (
                                                    venue.bookings.map(booking => (
                                                        <li key={booking.id} className="dropdown-item">
                                                            ...
                                                            <div><strong>Customer:</strong> <Link to={`/profile/${booking.customer.name}`}>{booking.customer.name}</Link></div>
                                                        </li>
                                                    ))
                                                ) : (
                    ...
                    <div className="d-flex justify-content-center">
                        <button className="btn btn-primary mt-3" onClick={() => setIsCreatingVenue(!isCreatingVenue)}>
                            {isCreatingVenue ? 'Hide' : 'Create New Venue'}
                        </button>
                        <button className="btn btn-warning mt-3 ms-2" onClick={handleDeregister}>De-register as Venue Manager</button>
                    </div>
                    {isCreatingVenue && (
                        <form onSubmit={handleCreateVenue} className="mt-3">
                            ...
                            <div className="mb-3">
                                <label htmlFor="mediaAlt" className="form-label">Media Alt Text</label>
                                <input
                                    type="text"
                                    className="form-control"
                                    id="mediaAlt"
                            ...
                            <div className="mb-3 form-check">
                                <input
                                    ...
                                    onChange={handleInputChange}
                                />
                                <label htmlFor="wifi" className="form-check-label">WiFi</label>
                            </div>
                            <div className="mb-3 form-check">
                                <input
                                    type="checkbox"
                ...
                <div className="text-center mt-3">
                    ...
                    <button className="btn btn-primary" onClick={onRegisterAsVenueManager}>Register as Venue Manager</button>
                </div>
            )}
        </div>
    );

...
function HomePage() {
    ...
    const fetchVenues = async (page, search) => {
        try {
            ...
            setTotalPages(Math.ceil(data.meta.totalCount / itemsPerPage)); // Adjust this based on the actual response structure
    ...
    const handleImageError = (e) => {
        e.target.src = placeholder; // Use the local placeholder image
    };
                    ...
                    <div className="row">
                        {venues.map(venue => (
                            <div key={venue.id} className="col-md-4 d-flex">
                                <Link to={`/venue/${venue.id}`} className="card mb-4 text-decoration-none text-dark">
                                    {venue.media && venue.media.length > 0 ? (
                                        ...
                                        <img src={placeholder} className="card-img-top" alt={venue.name} onError={handleImageError} />
                                    )}
...
const Pagination = ({ currentPage, totalPages, onPageChange }) => {
    const pagesToShow = 3; // Number of pages to show in the pagination component
